        ...
        <div className="not__ul">
          {this.state.heroImages.map((heroImage, index) => (
            <div key={index} className="not__li">
              <img
                src={this.getImgSrc(index + 1)}
                className={`about__panel__image__${index + 1} ${
                  this.state.firsClass ? 'load__in' : ' '
                }  ${this.state.secondClass ? 'panel__loaded' : ' '}`}
                ref={this.buildRef(index)}
                alt="Vector illustration of of a square"
                style={this.stylePanel(index)}
              />
            </div>
          ))}
        </div>
